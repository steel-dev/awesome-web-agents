...
- [authsome](https://github.com/agentrhq/authsome) - Local credential broker for AI agents. Encrypted vault and loopback HTTPS proxy inject OAuth2 tokens and API keys into outbound provider requests so web-agent runtimes never touch raw secrets. 45 providers bundled (14 OAuth2, 31 API key); browser PKCE / device code flows for first-time login. ![GitHub Repo stars](https://img.shields.io/github/stars/agentrhq/authsome?style=social)
